sinasc_consolidado <- readRDS("~/Demografia 1/sinasc_consolidado.rds")
...
library(readxl)
Popidadesexo_total <- read_excel("Demografia 1/Popidadesexo - total.xlsx")
Popidadesexo_mulheres <- read_excel("Demografia 1/Popidadesexo - mulheres.xlsx")
...
d18 <- data.frame("GRUPO ET?RIO" = c("15-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49"),freqnasc18 = d1$table$f)

d18<-d18 %>% inner_join(Popidadesexo_mulheres[c(1,2)], by = c('GRUPO.ET?RIO' = 'GRUPO ET?RIO'))
...
d19 <- data.frame("GRUPO ET?RIO" = c("15-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49"),freqnasc19= d2$table$f)

d19<-d19 %>% inner_join(Popidadesexo_mulheres[c(1,3)], by = c('GRUPO.ET?RIO' = 'GRUPO ET?RIO'))

##nascidos vivos de m?es em certas idades 2018
idademae20<-as.numeric(sinasc20$IDADEMAE)
d3 <- fdt (idademae20, start = 15, end = 50, h =5)
print(d3,columns = 1:2)
d20 <- data.frame("GRUPO ET?RIO" = c("15-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49"),freqnasc20 = d3$table$f)

d20<-d20 %>% inner_join(Popidadesexo_mulheres[c(1,4)], by = c('GRUPO.ET?RIO' = 'GRUPO ET?RIO'))
...
f18 <- data.frame("GRUPO ET?RIO" = c("15-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49"),freqnascfem18 = df1$table$f)
f18<-f18 %>% inner_join(Popidadesexo_mulheres[c(1,2)], by = c('GRUPO.ET?RIO' = 'GRUPO ET?RIO'))

idademae19_2<-as.numeric(sinasc19_2$IDADEMAE)
df2 <- fdt (idademae19_2, start = 15, end = 50, h =5)
print(df2,columns = 1:2)
f19 <- data.frame("GRUPO ET?RIO" = c("15-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49"),freqnascfem19 = df2$table$f)
f19<-f19 %>% inner_join(Popidadesexo_mulheres[c(1,3)], by = c('GRUPO.ET?RIO' = 'GRUPO ET?RIO'))

idademae20_2<-as.numeric(sinasc20_2$IDADEMAE)
df3 <- fdt (idademae20_2, start = 15, end = 50, h =5)
print(df3,columns = 1:2)
f20 <- data.frame("GRUPO ET?RIO" = c("15-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49"),freqnascfem20 = df3$table$f)
f20<-f20 %>% inner_join(Popidadesexo_mulheres[c(1,4)], by = c('GRUPO.ET?RIO' = 'GRUPO ET?RIO'))

Tabua_de_vida_2018_mulheres <- read_excel("Demografia 1/Tabua de vida 2018 - mulheres.xlsx")
Tabua_de_vida_2019_mulhers <- read_excel("Demografia 1/Tabua de vida 2019- mulhers.xlsx")
Tabua_de_vida_2020_mulhers <- read_excel("Demografia 1/Tabua de vida 2020 - mulhers.xlsx")
